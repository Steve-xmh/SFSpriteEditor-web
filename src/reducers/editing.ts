...
export const getSelectedColorIndex = (state: MainStore) =>
	state.editing.usingColorIndex;
export const getPreviewPalette = (state: MainStore) =>
	state.editing.previewPaletteId;
export const getHiddenSubsprites = (state: MainStore) =>
	state.editing.hiddenSubsprites;
...
export const isViewingAndSameId = (
	state: MainStore,
	viewType: typeof initialState["viewType"],
	id: number,
) => state.editing.id === id && state.editing.viewType === viewType;
export const isSubspriteVisible = (state: MainStore, id: number) =>
	!state.editing.hiddenSubsprites.has(id);
export const isPreviewTransparent = (state: MainStore) =>
	state.editing.previewTransparent;

export const switchToSprite = (id: number): SwitchAction => ({
	type: SWITCH_TO_SPRITE,
	id,
});
export const switchToSubsprite = (id: number): SwitchAction => ({
	type: SWITCH_TO_SUBSPRITE,
	id,
});
export const switchToTileset = (id: number): SwitchAction => ({
	type: SWITCH_TO_TILESET,
	id,
});
export const switchTool = (tool: Tools): SwitchToolAction => ({
	type: SWITCH_TOOL,
	tool,
});
export const switchColorIndex = (id: number): SwitchAction => ({
	type: SWITCH_COLOR_INDEX,
	id,
});
export const switchAnimation = (id: number): SwitchAction => ({
	type: SWITCH_ANIMATION,
	id,
});
export const switchAnimationFrame = (id: number): SwitchAction => ({
	type: SWITCH_ANIMATION_FRAME,
	id,
});
export const switchPreviewPalette = (id: number): SwitchAction => ({
	type: SWITCH_PREVIEW_PALETTE,
	id,
});
export const toggleHiddenSubsprite = (id: number): SwitchAction => ({
	type: TOGGLE_HIDDEN_SUBSPRITE,
	id,
});
export const togglePreviewTransparent = (): SwitchAction => ({
	type: TOGGLE_PREVIEW_TRANSPARENT,
	id: 0,
});
export const setTextToolText = (text: string): AnyAction => ({
	type: SET_TEXT_TOOL_TEXT,
	text,
});
export const setTextToolFont = (font: FontType): AnyAction => ({
	type: SET_TEXT_TOOL_FONT,
	font,
});
