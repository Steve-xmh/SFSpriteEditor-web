...
export const EditorCanvas: FunctionComponent = () => {
	...
	const [hitTestResult, setHitTestResult] = useState<HitTestResult | null>(
		null,
	);
	...
	const textToolFont = useSelector(getTextToolFont);
	...
	function getTarget(mousePosX?: number, mousePosY?: number) {
		if (canvasRef.current) {
			const offsetX =
				(((canvasRef.current.width - 256) / 2) | 0) + canvasView.x;
			const offsetY =
				(((canvasRef.current.height - 256) / 2) | 0) + canvasView.y;

			const targetX = Math.floor(
				((mousePosX || mousePos.x) - offsetX - 127) / canvasView.scale,
			);
			const targetY = Math.floor(
				((mousePosY || mousePos.y) - offsetY - 127) / canvasView.scale,
			);
	...
	function onCanvasMouseClick(evt: JSX.TargetedMouseEvent<HTMLCanvasElement>) {
		if (usingTool === Tools.ColorPicker) {
			const targetPos = getTarget(
				evt.clientX - evt.currentTarget.offsetLeft,
				evt.clientY - evt.currentTarget.offsetTop,
			);
			if (sprite) {
				switch (viewType) {
					case "sprite":
					...
					case "sprite": {
						...
						for (let y = 0; y < textCanvas.current.textData.height; y++) {
							for (let x = 0; x < textCanvas.current.textData.width; x++) {
								const pixel =
									textCanvas.current.textData.data[
										(y * textCanvas.current.textData.width + x) * 4 + 3
									];
								if (pixel === 255) {
									const test = hitTest({
										sprite: s,
	...
	function onCanvasMouseDown(evt: JSX.TargetedMouseEvent<HTMLCanvasElement>) {
		...
		if (usingTool === Tools.Pencil || usingTool === Tools.Eraser) {
			...
			usingTool === Tools.Rectangle ||
			usingTool === Tools.FilledRectangle ||
			usingTool === Tools.Line
		) {
			const targetPos = getTarget(
				evt.clientX - evt.currentTarget.offsetLeft,
				evt.clientY - evt.currentTarget.offsetTop,
			...
			if (hitTestResult) {
				const subsprite =
					sprite.sprites[viewId].subsprites[hitTestResult.subspriteId];
				if (subsprite) {
					const targetPos = getTarget(
						evt.clientX - evt.currentTarget.offsetLeft,
						evt.clientY - evt.currentTarget.offsetTop,
	...
	function onCanvasMouseMove(evt: JSX.TargetedMouseEvent<HTMLCanvasElement>) {
		if (usingTool === Tools.Pencil || usingTool === Tools.Eraser) {
	...
	function onCanvasMouseUp(evt: JSX.TargetedMouseEvent<HTMLCanvasElement>) {
		if (evt.button !== 0) return;
		if (usingTool === Tools.Pencil || usingTool === Tools.Eraser) {
			if (sprite) {
				switch (viewType) {
					case "sprite":
						if (sprite.sprites[viewId]) {
							...
							dispatch(
								setPixels(
									usingTool === Tools.Eraser ? 0 : colorIndex,
									s.tileSetID,
									pixelMap,
								),
							);
				...
				switch (viewType) {
					case "sprite":
						if (sprite.sprites[viewId] && toolTempRef.current) {
							const subsprite =
								sprite.sprites[viewId].subsprites[
									toolTempRef.current.movingSubsprite.subspriteId
								];
							const targetPos = getTarget(
								evt.clientX -
									evt.currentTarget.offsetLeft +
									toolTempRef.current.x,
								evt.clientY -
									evt.currentTarget.offsetTop +
									toolTempRef.current.y,
							);
							...
							newSubsprites[toolTempRef.current.movingSubsprite.subspriteId] =
								newSubsprite;
	...
	function onRedraw() {
		...
		if (canvas) {
			...
			const offsetX =
				(((canvas.width - 256) / 2) | 0) + Math.round(canvasView.x);
			const offsetY =
				(((canvas.height - 256) / 2) | 0) + Math.round(canvasView.y);
						...
						if (sprite.sprites[viewId]) {
							...
							if (showSubSpriteBounds) {
								for (const [i, subsprite] of s.subsprites.entries()) {
									...
									ctx.strokeRect(
										subsprite.position.x,
										subsprite.position.y,
										subsprite.size.x,
										subsprite.size.y,
									);
								}
			...
			if (hitTestResult) {
				printDebug(
					`Hit Test: ${hitTestResult.tileId} ${hitTestResult.pixelIndex}`,
				);
		...
		if (canvas) {
			if (evt.ctrlKey) {
				...
				setCanvasView((prev) => ({
					...
					scale: Math.max(
						1,
						Math.min(50, prev.scale - (evt.deltaY > 0 ? 0.2 : -0.2)),
					),
				}));
			} else {
				// Scroll the canvas
