...
export const Palette: FunctionComponent<{ id: number }> = (props) => {
	...
	const onPasteReplacePalette = async () => {
		...
		if (text.startsWith("PALETTE")) {
			...
			if (
				palette instanceof Array &&
				palette.length === sprite.palettes[props.id].length
			) {
	...
	return (
		<div className={styles.palette}>
			...
			<button className={styles.paletteButton}>
				<img src={arrowUpSvg} onClick={onCopyPalette} />
			</button>
			<button className={styles.paletteButton}>
				<img src={arrowDownSvg} onClick={onPasteReplacePalette} />
			</button>
			<button className={styles.paletteButton}>
				<img src={removeSvg} />
			</button>
...
export const PalettePage: FunctionComponent = (props) => {
	const palettes = useSelector<MainStore, Color[][]>(
		(state) => getSprite(state).palettes,
	);

	return (
		<div className={styles.palettesPage}>
			<button>
				<FormattedMessage
					id="palettes.toggle-color-mode"
					defaultMessage='Toggle color mode'
				/>
			</button>
			<button>
				<FormattedMessage
					id="palettes.new-palette"
					defaultMessage='New Palette'
				/>
			</button>
			{palettes.map((_color, i) => (
				<Palette key={i} id={i} />
			))}
		</div>
	);
