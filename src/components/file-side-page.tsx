...
import { addFile, addOpenError, clearOpenError, closeAllFiles, closeFile, getFiles, setLoading } from '../reducers/files'
...
export const FileSidePage: FunctionComponent = props => {
            ...
            if (tempCanvasRef.current) {
                if (sprite.animations[0]) {
                    ...
                    if (frame) {
                        ...
                        const imageData = ctx.createImageData(tempCanvasRef.current.width, tempCanvasRef.current.height)
                        renderSprite({
                            sprite: sprite.sprites[frame.spriteId],
                            tileset: sprite.tilesets[sprite.sprites[frame.spriteId].tileSetID],
                            palette: sprite.palettes[frame.palette],
                            putPixelCallback: (x, y, color) => {
                                const index = ((y - bound.top) * (tempCanvasRef.current.width) + (x - bound.left)) * 4
        ...
        if (supportFilePicker) {
            ...
            if (files.length > 0) {
                ...
                if (files.length === 1) {
                    ...
                    const isAllSuccess = (await Promise.all(promises)).reduce((acc, cur) => acc && cur, true)
                    if (isAllSuccess) {
                        dispatch(closeDialog('loading'))
                    }
                }
    ...
    return <div className={styles.filePage}>
        <div className={styles.fileButtons}>
            ...
            <input type="file" ref={openInputRef} onChange={onOpenFilesInputChanged} style={{ display: 'none' }} />
