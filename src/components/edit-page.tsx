...
import {
	...
	switchColorIndex,
	switchTool,
	toggleHiddenSubsprite,
	togglePreviewTransparent,
	Tools,
...
export const PaletteColorSwitcher: FunctionComponent = (props) => {
	...
	const colorIndex = useSelector<MainStore, number>(
		(state) => state.editing.usingColorIndex,
	);
	...
	return (
		<div className={styles.paletteColorSwitcher}>
		...
		<div className={styles.editPage}>
			<div className={styles.toolbar}>
				...
				<ToolButton tool={Tools.Rectangle} shortcut='A'>
					<FormattedMessage
						id="edit.tool.rectangle"
						defaultMessage='Rectangle'
					/>
				</ToolButton>
				<ToolButton tool={Tools.FilledRectangle} shortcut='S'>
					<FormattedMessage
						id="edit.tool.filled-rectangle"
						defaultMessage='Filled Rectangle'
					/>
				</ToolButton>
				...
				<ToolButton tool={Tools.MoveSubsprite} shortcut='M'>
					<FormattedMessage
						id="edit.tool.move-subsprite"
						defaultMessage='Move Subsprite'
					/>
				</ToolButton>
			</div>
			...
			<div className={styles.options}>
				{usingTool === Tools.Text && (
					<>
						<label>
							<FormattedMessage id="edit.editing.font" defaultMessage='Font' />
						</label>
						<select
							onChange={(evt) =>
								dispatch(
									setTextToolFont(
										parseInt(evt.currentTarget.selectedOptions.item(0).value),
									),
								)
							}
						>
						...
						<label>
							<FormattedMessage id="edit.editing.text" defaultMessage='Text' />
						</label>
						<input
							ref={textareaRef}
							value={textToolText}
							onInput={(evt) =>
								dispatch(setTextToolText(evt.currentTarget.value))
							}
						></input>
					</>
				)}
				<label>
					<FormattedMessage
						id="edit.show-zero-index-color"
						defaultMessage='Show zero index color'
					/>
				</label>
				<input
					type='checkbox'
					onClick={() => dispatch(togglePreviewTransparent())}
			...
			{viewType === "sprite" && (
				<div className={styles.subspriteList}>
					{currentId in sprite.sprites &&
						sprite.sprites[currentId].subsprites.map((subsprite, i) => (
							<div key={i} className={classname(styles.subsprite)}>
								{i}
								<SubSprite
									transparent={false}
									spriteId={currentId}
									subspriteId={i}
									paletteId={previewPalette}
								/>
								<input
									type='checkbox'
									onClick={() => dispatch(toggleHiddenSubsprite(i))}
									checked={!hiddenSubsprites.has(i)}
								></input>
							</div>
						))}
				</div>
