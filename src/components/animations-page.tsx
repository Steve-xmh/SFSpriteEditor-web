...
export const AnimationsPage: FunctionComponent = () => {
    ...
    const previews = useMemo(() => {
        return animations.map(animation => useMemo(() => {
            ...
            const bounding = animation.map(frame => getSpriteBound(sprite.sprites[frame.spriteId].subsprites)).reduce((a, b) => {
                return {
                ...
                if (animation.length > 0) {
                    ...
                    if (currentFrame) {
                        ...
                        if (frameTimer >= currentFrame.delay / 60 * 1000) {
                            const nextFrame = currentFrame.isLoop ? 0 : ((frameIndex + 1) % animation.length)
    ...
    return <div className={styles.animationsPage}>
        <div className={styles.animationsList}>
            {
                animations.map((animation, index) =>
                    <button
                        data-index={index}
                        key={index}
                        className={classname(styles.animationsButton, editing.selectedAnimationId === index && styles.selected)}
                        onClick={() => {
                    ...
                    >
                        <span>{index}</span>
                        <img src={
                            previews[index][hoveringAnimation === index ? currentFrameIndex : 0]?.preview ||
            ...
            <div className={styles.frameOptions}>
                <div className={styles.buttonsPane}>
                    <button onClick={onNewAnimation}>
                        <FormattedMessage id='animations.new-animation' defaultMessage='New Animation' />
                    </button>
                    <button onClick={onDeleteAnimation} disabled={!(editing.selectedAnimationId in sprite.animations)}>
                        <FormattedMessage id='animations.delete-selected-animation' defaultMessage='Delete Selected Animation' />
                    </button>
                    <button onClick={onNewAnimationFrame} disabled={!(editing.selectedAnimationId in sprite.animations)}>
                        <FormattedMessage id='animations.new-animation-frame' defaultMessage='New Frame' />
                    </button>
                    <button onClick={onDeleteAnimationFrame} disabled={
                        !(editing.selectedAnimationId in sprite.animations) ||
                        !(editing.selectedAnimationFrame in sprite.animations[editing.selectedAnimationId])
                    }>
                        <FormattedMessage id='animations.delect-selected-animation-frame' defaultMessage='Delete Selected Frame' />
                    </button>
                    <button onClick={onMoveUpAnimationFrame} disabled={
                        !(editing.selectedAnimationId in sprite.animations) ||
                        !(editing.selectedAnimationFrame in sprite.animations[editing.selectedAnimationId]) ||
                        editing.selectedAnimationFrame === 0
                    }>
                        <FormattedMessage id='animations.move-up-selected-animation-frame' defaultMessage='Move Up Frame' />
                    </button>
                    <button onClick={onMoveDownAnimationFrame} disabled={
                        !(editing.selectedAnimationId in sprite.animations) ||
                        !(editing.selectedAnimationFrame in sprite.animations[editing.selectedAnimationId]) ||
                        editing.selectedAnimationFrame === sprite.animations[editing.selectedAnimationId].length - 1
                    }>
                        <FormattedMessage id='animations.move-down-selected-animation-frame' defaultMessage='Move Down Frame' />
                    </button>
                </div>
                <div>
                    <b><FormattedMessage id='animations.frame-config' defaultMessage='Frame Config' /></b>
                </div>
                <div className={styles.frameConfig}>
                    <label><FormattedMessage id='animations.loop' defaultMessage='Loop' /></label><input type="checkbox" onClick={onInputLoop} checked={getCurrentAnimationFrameLoop()}></input>
                    <label><FormattedMessage id='animations.sprite-id' defaultMessage='Sprite ID' /></label><input type="number" min="0" max={sprite.sprites.length - 1} onInput={onInputSpriteId} value={getCurrentAnimationFrameSpriteId()}></input>
                    <label><FormattedMessage id='animations.palette-id' defaultMessage='Palette ID' /></label><input type="number" min="0" max={sprite.palettes.length - 1} onInput={onInputPaletteId} value={getCurrentAnimationFramePaletteId()}></input>
                    <label><FormattedMessage id='animations.delay' defaultMessage='Delay' /></label><input type="number" min="0" max="255" onInput={onInputDelay} value={getCurrentAnimationFrameDelay()}></input>
                </div>
            </div>
            <div className={styles.animationFrameList}>
                {
                    animations[editing.selectedAnimationId] && animations[editing.selectedAnimationId].map((frame, index) =>
                        <button
                            data-index={index}
                            key={index}
                            className={classname(styles.animationsButton, editing.selectedAnimationFrame === index && styles.selected)}
                            onClick={() => {
                        ...
                        >
                            <span>{index}</span>
                            <img src={
                                previews[editing.selectedAnimationId][index].preview
                            } />
                        </button>
                    )
                }
